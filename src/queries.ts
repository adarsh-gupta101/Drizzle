import db from "./db";
import { cities, countries, City, Country,State,states } from "./schema";
...
// export async function statesm() {
//   const state = await db.query.states.findMany({
//     with: {
//       country: true,
//     },
//   });
...
// adds a new state
export async function insertState(newStates: State[]): Promise<State[]> {
  const insertedStates = await db.insert(states).values(newStates).returning();
